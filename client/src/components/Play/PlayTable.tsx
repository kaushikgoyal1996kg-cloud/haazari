...
export function PlayTable() {
  const { room, gameState, myPlayerId, myArrangedSets, playSet, gameError, clearGameError, voiceParticipants, speakingPlayerIds } = useGame();
          ...
          return (
            <div key={pid} className={`seat seat--${side} ${isMe ? 'seat--me' : ''} ${isTurn ? 'seat--turn' : ''}`}>
              <div className="seat__badge-row">
                ...
                {voiceParticipants.includes(pid) && (
                  <span
                    className={`seat__badge seat__badge--voice ${
                      speakingPlayerIds.includes(pid) ? 'seat__badge--voice-speaking' : ''
                    }`}
                  >
                    🎙️
                  </span>
