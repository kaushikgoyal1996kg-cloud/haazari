...
export function ArrangementScreen({ hand, onConfirm, onDismiss, submitError, cumulativeScore }: Props) {
  ...
  const dismissEligible = useMemo(() => {
    ...
    // Checked directly on the raw dealt hand - true from the moment it's
    // dealt, regardless of how much (or how little) the player has
    // arranged so far. See handHasNoPossibleSequence's doc for why this is
    // the correct check rather than looking at the current draft slots.
  ...
  function handleDismiss() {
    // The proposed arrangement just needs to use every dealt card with the
    // right sizes (3,3,3,4) - ordering doesn't matter for a dismissal
    // claim, so if the player hasn't (or hasn't fully) arranged their
    // hand manually, fall back to a simple mechanical split rather than
    // forcing them to finish arranging a hand they're about to void anyway.
